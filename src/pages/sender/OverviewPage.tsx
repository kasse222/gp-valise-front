...
import { ArrowRight, Search, Clock, AlertCircle, Package } from 'lucide-react'
...
function StatCard({ label, value, variant }: {
  ...
  const colors = { primary: 'text-[#1B3A6B]', danger: 'text-red-600', success: 'text-emerald-600' }
  return (
    <Card>
...
export default function OverviewPage() {
  ...
  const recent          = [...bookings].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 3)
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      {pendingApproval.length > 0 && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-[14px] flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-amber-700 mt-0.5">Le paiement sera disponible une fois le voyageur a accepté votre demande.</p>
            <Link to="/sender/bookings" className="inline-block mt-2 text-xs font-semibold text-amber-800 hover:underline">Voir mes réservations →</Link>
          </div>
        </div>
      )}
      ...
      {pendingPayment.length > 0 && (
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-[14px] flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-blue-700 mt-0.5">Finalisez votre paiement avant expiration du délai.</p>
            <Link to="/sender/bookings" className="inline-block mt-2 text-xs font-semibold text-blue-800 hover:underline">Payer maintenant →</Link>
          </div>
        </div>
      )}
      ...
      {recent.length === 0 ? (
        <EmptyState icon={Package} title="Aucune réservation" description="Explorez les trajets disponibles pour envoyer votre premier colis." />
      ) : (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-gray-900">Réservations récentes</h3>
            <Link to="/sender/bookings" className="text-xs text-[#1B3A6B] hover:underline font-medium">Voir tout</Link>
          </div>
                ...
                <Card className="hover:shadow-md transition-shadow cursor-pointer p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="font-medium text-gray-900 text-sm truncate">{booking.trip?.departure ?? '—'}</span>
                      <ArrowRight size={14} className="shrink-0 text-gray-400" aria-hidden />
                      <span className="font-medium text-gray-900 text-sm truncate">{booking.trip?.destination ?? '—'}</span>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <StatusBadge code={booking.status as BookingStatusCode} />
                      <span className="text-xs text-gray-400 hidden sm:inline">{formatDate(booking.created_at)}</span>
                    </div>
                  </div>
                  {booking.status === 'en_paiement' && (
                    <p className="text-xs text-blue-600 mt-2 font-medium">⏱ Paiement requis avant expiration</p>
                  )}
                </Card>
              </Link>
            ))}
