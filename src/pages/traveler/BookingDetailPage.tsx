...
export default function TravelerBookingDetailPage() {
  ...
  // F-030 — devise depuis les items (prix_par_kg × devise du trajet)
  // Le trip porte la devise ; fallback XOF (corridor Africa majoritaire)
  const tripCurrency: string = (booking.trip as any)?.currency ?? 'XOF'
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <Card className="mb-4">
        ...
        {/* F-030 — devise du trajet, pas EUR hardcodé */}
        <p className="text-xl font-bold text-[#1B3A6B] font-mono">
          {formatAmount(totalAmount, tripCurrency)}
        </p>
      </Card>
              ...
              return (
                <div key={item.id} className="flex flex-col gap-3 p-3 bg-gray-50 rounded-[12px]">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {/* F-030 — devise du trajet */}
                    <span className="font-bold text-gray-900 font-mono shrink-0">
                      {formatAmount(item.price, tripCurrency)}
                    </span>
                  </div>
