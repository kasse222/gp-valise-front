...
export default function PaymentsPage() {
  ...
  const is403 =
    isError && (error as AxiosError)?.response?.status === 403;
        ...
        <Card>
          <div className="flex items-center gap-3 text-amber-600 py-2">
            ...
            <p className="text-sm font-medium">
              Vérification email ou KYC requis pour accéder à vos paiements.
            </p>
          </div>
        </Card>
      </div>
  ...
  const transactions = (data ?? []).filter((tx) =>
    SHOWN_TYPES.has(tx.type.code)
  );

  const completedPayouts = transactions.filter(
    (tx) => tx.type.code === "payout" && tx.status.is_success
  );
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <Card className="mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500">Total versements reçus</p>
            <p className="text-2xl font-bold text-green-600">
              {formatAmount(totalCents, totalCurrency)}
            </p>
          </div>
        </div>
      </Card>
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    transactionTypeColor[tx.type.code] ??
                ...
                <span
                  className={cn(
                    "inline-flex items-center px-2 py-0.5 rounded text-xs font-medium",
                    transactionStatusColor[tx.status.code] ??
                      "bg-gray-100 text-gray-700"
                  )}
                >
                  {tx.status.label}
