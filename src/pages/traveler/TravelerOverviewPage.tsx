...
import { ArrowRight, Home, Plus, Package } from 'lucide-react'
...
export default function TravelerOverviewPage() {
  ...
  // Instant Booking — colis en transit sur mes trajets (remis, pas encore livrés)
  const inTransit = bookings.filter(
    (b) => b.status === 'en_transit' && myTripIds.has(b.trip_id),
  )
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      {/* ── Colis en transit — action requise ─────────────────────────── */}
      {inTransit.length > 0 && (
              ...
              <Link key={booking.id} to={`/traveler/bookings/${booking.id}`}>
                <Card className="hover:shadow-md transition-shadow cursor-pointer flex flex-col gap-2">
                  ...
                  <p className="text-xs text-gray-500">
                    {booking.user?.full_name ?? booking.user?.email ?? 'Expéditeur'}
                  </p>
