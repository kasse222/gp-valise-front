...
export const WORLD_CITIES = [
  ...
  'Madrid', 'Barcelone', 'Valence', 'Séville', 'Bilbao', 'Malaga', 'Las Palmas',
  ...
  'Londres', 'Birmingham', 'Manchester', 'Glasgow', 'Édimbourg', 'Liverpool', 'Leeds', 'Bristol',
  'Stockholm', 'Oslo', 'Copenhague', 'Helsinki',
  'Varsovie', 'Prague', 'Budapest', 'Bucarest', 'Vienne', 'Athènes', 'Sofia', 'Zagreb', 'Belgrade', 'Kiev',
  ...
  'Cayenne', 'Port-au-Prince', 'Kingston', 'Port of Spain', 'Santo Domingo',
  'São Paulo', 'Rio de Janeiro', 'Buenos Aires', 'Santiago', 'Lima', 'Bogotá', 'Caracas', 'Quito',
  'Dubaï', 'Abou Dhabi', 'Riyad', 'Djeddah', 'Koweït', 'Doha',
  'Manama', 'Mascate', 'Beyrouth', 'Amman', 'Bagdad', 'Téhéran', 'Istanbul', 'Ankara', 'Izmir',
  'Mumbai', 'Delhi', 'Bangalore', 'Chennai', 'Kolkata', 'Karachi', 'Lahore', 'Dhaka', 'Colombo', 'Katmandou',
  'Bangkok', 'Kuala Lumpur', 'Singapour', 'Jakarta', 'Manille', 'Hô Chi Minh-Ville', 'Hanoi', 'Phnom Penh', 'Rangoun',
  'Pékin', 'Shanghai', 'Canton', 'Shenzhen', 'Hong Kong', 'Taipei', 'Séoul', 'Busan', 'Tokyo', 'Osaka', 'Nagoya',
  'Sydney', 'Melbourne', 'Brisbane', 'Perth', 'Auckland',
] as const

function getCities(countryCode?: string): readonly string[] | string[] {
  if (!countryCode) return WORLD_CITIES
  const found = COUNTRIES.find((c) => c.code === countryCode)
  return found ? found.cities : WORLD_CITIES
}
...
export function CitySelect({
  value, onChange, placeholder = 'Ville',
  label, error, className, required, id, countryCode,
}: CitySelectProps) {
  const uid      = useId()
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 w-full', className)}>
      ...
      <div className="relative flex items-center">
        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 shrink-0 pointer-events-none" aria-hidden>
          <MapPin className="h-4 w-4" />
        </span>
        <input
          id={inputId} type="text" list={listId} value={value}
          ...
          className={cn(
            'w-full min-h-[48px] pl-10 pr-4 py-3 rounded-[10px] border bg-white text-sm text-gray-900',
...
export function CityInputInline({ value, onChange, placeholder = 'Ville', className }: CityInputInlineProps) {
  const uid    = useId()
  ...
  return (
    <div className={cn('flex items-center gap-2 flex-1 min-w-0', className)}>
      ...
      <datalist id={listId}>
        {WORLD_CITIES.map((city) => <option key={city} value={city} />)}
      </datalist>
    </div>
  )
}